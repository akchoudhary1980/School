// Add Student Model
function ShowModel() {
    $('#add-student-lg').modal('show');
}
// View Student Model
function ShowModel() {
    $('#update-student-lg').modal('show');
}
// Student Auto complete function 
$(document).ready(function () { 
    $(document).ready(function () {
        $("#StudentName").autocomplete({
            source: function (request, response) {
                $.ajax({
                    url: "/Share/StudentAutoComplete",
                    type: "POST",
                    dataType: "json",
                    data: { Prefix: request.term },
                    success: function (data) {
                        response($.map(data, function (item) {
                            return { label: item.StudentName, value: item.StudentName, id: item.StudentID };
                        }))
                    },
                })
            },
...
// Class Auto complete function
// Board Auto complete function

// Activity Auto complete function
$(document).ready(function () {   
    $(document).ready(function () {
        $("#ActivityName").autocomplete({
            source: function (request, response) {
                $.ajax({
                    url: "/Share/ActivityAutoComplete",
                    type: "POST",
                    dataType: "json",
                    data: { Prefix: request.term },
                    success: function (data) {
                        response($.map(data, function (item) {
                            return { label: item.ActivityName, value: item.ActivityName, id: item.ActivityID };
                        }))
                    },
                })
            },
            select: function (event, ui) {
                $("#ActivityName").val(ui.item.label);
            },
        });
    })
})
